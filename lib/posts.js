...
const postsDirEn = path.join(process.cwd(), 'content/posts/en');
...
const processPost = (fileName, rawContent) => {
  ...
  try {
    ...
    return {
      ...
      seriesOrder: attributes.seriesOrder ? Number(attributes.seriesOrder) : null,
    };
  } catch (e) {
...
export function getPostByIdEn(id) {
  const filePath = path.join(postsDirEn, `${id}.md`);
  if (!fs.existsSync(filePath)) return null;
  const raw = fs.readFileSync(filePath, 'utf-8');
  return processPost(`${id}.md`, raw);
}

export function hasEnglishVersion(id) {
  return fs.existsSync(path.join(postsDirEn, `${id}.md`));
}

export function getAllEnglishPostIds() {
  if (!fs.existsSync(postsDirEn)) return [];
  return fs.readdirSync(postsDirEn)
...
export async function getAllPostsWithComments() {
  ...
  return getAllPosts().map((post) => ({
    ...post,
    hasComments: (commentCounts[post.id] || 0) > 0,
  }))
}